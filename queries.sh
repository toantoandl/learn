...
echo "$($PSQL "SELECT teams.name FROM teams
               RIGHT JOIN games ON teams.team_id=games.winner_id
               WHERE games.round='Final' AND games.year=2018
               LIMIT 1")"
...
echo "$($PSQL "SELECT DISTINCT name FROM teams
               RIGHT JOIN games ON teams.team_id=games.winner_id ORDER BY name ASC")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year,name FROM teams
               RIGHT JOIN games ON teams.team_id=games.winner_id
               WHERE games.round='Final' ORDER BY name DESC")"
